...
#reading gilmor homes parcel shapefile
gilmor_homes <- st_read("data/gilmor-homes.geojson")
...
recent_shootings_by_district %>%
  filter(district %in% c("western"))

```

## Gun violence probably isn’t shifting from the Western District into other parts of Baltimore.
...
## A heavier police presence isn’t the full story in the Western, but other factors may be at play.
...
"Though the Western had the second highest arrest total of any district in 2022, it has been on a steady downward slope since 2010, when almost 9 times as many arrests were made."
...
"There were a relatively high number of shooting victims in those two months, but overall the year was not an outlier. Year-over-year, the Western saw a 35% decline in shooting victims."
...
"Even if you compared 2022 to the year with the fewest victims since 2015, the Western still recorded a 24% drop."
...
"Partially demolished in 2020, the number of shootings that occurred in and around the complex where Freddie Gray lived had already fallen significantly in 2020 and 2021, meaning lower crime in the area was already baked into the data before the pilot began last year."

```{r}

near_gilmor <- function(blocks){
  
  distance <- blocks*100+50  

  near_complex <- as.data.frame(st_is_within_distance(western_shootings, gilmor_homes, dist = distance, sparse = FALSE)) %>%
    clean_names() %>%
  ...
  map <- mapView(gilmor_homes$geometry, alpha.regions = 0, color = "red", lwd = 2, layer.name = "gilmor Homes") + 
  ...
  plot <- ggplot(near_complex_by_year, aes(x=year, y=shootings)) +
    geom_line() +
    ggtitle(paste0("Shootings within ", blocks, " block(s) of gilmor Homes"))
...
```
"Shootings within about two, three and four blocks of Gilmor Homes dropped by almost half in 2021 even as shootings in the Western District increased, finishing higher than any year since 2017."
...
"Of the eight districts that saw an average population loss, The Banner found some with large decreases in population and small decreases in shootings; others showed large decreases in population and increases in shootings."
...
"The effect of population loss in the Western does not seem to be fewer shootings, but fewer people to witness the violence that remains. The Western already had the highest gunshot victim rate of any district with 4.5 victims per 1,000 residents. With five years of population loss and a 6% decrease in the number of shootings, the rate increased to 5.4."
